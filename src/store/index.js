...
import {calcCoverage, getGenomeCoverage} from "@/utils/processCSVfile";
            ...
            if (state.processedFiles.length === payload.filesCount) {
                ...
                state.processedFiles.sort((a, b) => {
                    if (a.seqDetails.dataset.localeCompare(b.seqDetails.dataset) > 0) {
                        return 1
                    } else if (a.seqDetails.dataset.localeCompare(b.seqDetails.dataset) < 0) {
                        return -1
                    } else {
                ...
                state.processedFiles.forEach((file) => {
                    ...
                    // set number of initial reads as not all datasets had 10 mil reads, those that did were set automatically to 10 mils
                    ...
                    file.seqDetails.reads.filter(read => read.variantSpecific).forEach(read => calcCoverage(read, file.seqDetails.ref.coverage.strainSpecific))

                    file.seqDetails.ref.coverage.strainSpecific.genomeCov = getGenomeCoverage(file.seqDetails.ref.seqLength, file.seqDetails.ref.coverage.strainSpecific.nonredundant)
